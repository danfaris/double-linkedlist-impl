...
public class DLList {
	...
	public void remove(int index) {
		if (size > 0) {
			...
			// If current item is the head, then the head become the next right node after
			// current
			if (curr == head) {
				head = curr.getNext();
				head.setPrev(null);
			}
			// If current item is the last, then the last become the previous right before
			// current
			else if (curr == last) {
				last = curr.getPrev();
				last.setNext(null);
			} else {
				// If current item is not the last or the first, then the element to be removed
				// is in the middle
	...
	// the information from the nodes starting from the last node up to the first
	// node */
	// ************************************************************/
